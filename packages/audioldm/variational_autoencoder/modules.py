    ...
    def __init__(
        ...
        **ignore_kwargs,
        ...
        for i_level in range(self.num_resolutions):
            block = nn.ModuleList()
            attn = nn.ModuleList()
            block_in = ch * in_ch_mult[i_level]  # ch:输入通道数量
            block_out = ch * ch_mult[i_level]
            for i_block in range(self.num_res_blocks):
                block.append(
                    ResnetBlock(
                ...
                block_in = block_out #会在将层数提上来后，持续在同一个层数上卷积。然后持续num_res_blocks层
                if curr_res in attn_resolutions:
                    attn.append(make_attn(block_in, attn_type=attn_type))  # 在照片的特征维度上进行self-attention
        ...
        # 下采样由：num_resolutions层构成，每层内由：
        # 1.num_res_blocks层卷积与单层在像素级特征上的attn
        # 2.每层都会下采样为原来的1/2
        # 不同大层内的卷积channel不同，相同层内卷积channle相同。
        ...
        # middle层由单层卷积，单层self-attention，单层卷积构成
