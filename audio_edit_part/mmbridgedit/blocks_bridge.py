...
class PatchEmbed1D(nn.Module):
    ...
    def forward(self, x, Tx):
        B, C, L = x.shape  # 输入形状: (batch_size, in_chans, sequence_length)
        Lx = (L + self.patch_size - 1)//self.patch_size
        pad = (self.patch_size - L % self.patch_size) % self.patch_size + (Lx//Tx - Lx % Tx ) % (Lx//Tx)*self.patch_size# 计算填充大小
        lift_pad = pad // 2
        right_pad = pad-lift_pad
        x = F.pad(x, (lift_pad, right_pad))  # 在序列末尾填充
        x = self.proj(x)  # 卷积操作
        if self.flatten:
            x = x.permute(0,2,1)
            x = self.norm(x)  # 归一化
        return x,lift_pad,right_pad
...
class PatchEmbed2D(nn.Module):
    ...
    def forward(self, x, Tw: Optional[int] = None):
        # 注意，这里的H:单时间不同频率特征
        # W：时间轴
        B, C, H, W = x.shape
        ...
        # 第二级填充（当指定Tw时）：确保分块数可被Tw整除
        if Tw is not None:
            Wx = (W + pad_w) // pw  # 基础分块数
            delta = (Tw - (Wx % Tw)) % Tw  # 需要增加的块数
            pad_w += delta * pw  # 追加填充量
            
        # 执行非对称填充（仅在宽度方向）
        pad_left = pad_w // 2
        pad_right = pad_w - pad_left
        x = F.pad(x, (pad_left, pad_right, 0, 0))  # (left, right, top, bottom)
        
        # 验证高度是否符合要求（当strict模式时）
        if self.strict_img_size and (H % ph != 0):
            raise ValueError(f"输入高度{H}必须能被patch高度{ph}整除")
        
        # 执行卷积投影
        x = self.proj(x)  # (B, C, H//ph, W_new//pw)
        
        if self.norm is not None:
            Wh, Ww = x.size(2), x.size(3)
            x = x.flatten(2).transpose(1, 2) 
            x = self.norm(x)
            x = x.transpose(1, 2).view(-1, self.embed_dim, Wh, Ww) # B, C, H ,W
        x = x.permute(0,1,3,2)# B, C, W, H 
        # 展平处理
        if self.flatten:
            # B, C, W, H
            x = x.flatten(2).transpose(1, 2)  # (B, num_patches(W*H), C)
  
        return x, pad_left, pad_right # 返回的结果是 B, C, W, H 或者是 (B, num_patches(W*H), C) 
